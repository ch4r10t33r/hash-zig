...
fn main() {
    ...
    println!("=== Rust Poseidon2 Direct Comparison ===");
    ...
    println!("\n=== Test Poseidon2 Tree Hash with Exact Zig Inputs ===");
    
    // Test with the exact inputs that Zig is using
    let test_input = vec![
        0x1640cb16, 0x54503ce2, 0x7e118cb3, 0x6aeeecb5, 0x4ea08a17, 0x2c138707, 0x65d14fc6, 0x2c5e70b5,
        0x30ff8f32, 0x59e166e4, 0x7e8fc675, 0x60080f45, 0x5bbb59d8, 0x5d5742ec, 0x1e0d8135, 0x4915976b
    ];
    ...
    // Test tweak computation for level 5, position 0
    let level: u8 = 5;
    ...
    let tweak = [
        (tweak_bigint % p) as u32,
        ((tweak_bigint / p) % p) as u32,
    ];
    ...
    println!("Expected Zig result: 0x646a2743");
    println!("Actual Rust result: 0x{:x}", root_values[0]);
    println!("Match: {}", root_values[0] == 0x646a2743);
    ...
    println!("The issue appears to be in the Poseidon2 hash function implementation itself.");
    println!("Same inputs produce different outputs between Rust and Zig implementations.");
    println!("This suggests there are still subtle differences in our Plonky3-compatible implementation.");
}

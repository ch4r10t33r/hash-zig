...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 2 {
        eprintln!("Usage:");
        eprintln!("  {} keygen [seed_hex] [lifetime]        - Generate keypair (lifetime: 2^8, 2^18, or 2^32, default: 2^8)", args[0]);
        eprintln!("  {} sign <message> <epoch>               - Sign message using tmp/rust_sk.json, save to tmp/rust_sig.bin", args[0]);
    ...
    match args[1].as_str() {
        "keygen" => {
            ...
            keygen_command(seed_hex, lifetime)?;
        }
        "sign" => {
            if args.len() < 4 {
            ...
            sign_command(message, epoch, lifetime)?;
        }
        "verify" => {
            if args.len() < 6 {
            ...
            let lifetime = LifetimeTag::from_file()?;
            verify_command(sig_path, pk_path, message, epoch, lifetime)?;
        }
        _ => {
...
fn keygen_command(seed_hex: Option<&String>, lifetime: LifetimeTag) -> Result<(), Box<dyn std::error::Error>> {
...
fn sign_command(message: &str, epoch: u32, lifetime: LifetimeTag) -> Result<(), Box<dyn std::error::Error>> {
...
fn verify_command(sig_path: &str, pk_path: &str, message: &str, epoch: u32, lifetime: LifetimeTag) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Verify the signature
        ...
        LifetimeTag::Pow18 => {
            let signature: <SIGTopLevelTargetSumLifetime18Dim64Base8 as SignatureScheme>::Signature = bincode::deserialize(sig_data)?;
            
            // Load public key from Zig
            let pk_json = fs::read_to_string(pk_path)?;
            type PkType = <SIGTopLevelTargetSumLifetime18Dim64Base8 as SignatureScheme>::PublicKey;
            let public_key: PkType = serde_json::from_str(&pk_json)?;

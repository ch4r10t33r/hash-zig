...
fn main() {
    ...
    // Compare serialized public keys (bincode) for exact equality
    let pk1_bytes = bincode::serialize(&pk1).expect("serialize pk1");
    let pk2_bytes = bincode::serialize(&pk2).expect("serialize pk2");

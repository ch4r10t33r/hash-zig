...
use hashsig::symmetric::tweak_hash::{chain as tweak_chain, TweakableHash};
...
fn main() {
    ...
    if let Some(sig_obj) = sig_val.as_object_mut() {
    ...
    hex_array_to_numbers(&mut pk_val);
    hex_array_to_numbers(&mut sig_val);
    ...
    let sig_built: <SIGTopLevelTargetSumLifetime32Dim64Base8 as SignatureScheme>::Signature =
        match serde_json::from_value(sig_val.clone()) {
            Ok(v) => v,
            Err(e) => {
    ...
    if env::var("HASH_SIG_DEBUG_POSEIDON").is_ok() {
        ...
        if let (Some(pk_dbg_val), Some(sig_dbg_val)) = (pk_dbg.as_ref(), sig_dbg.as_ref()) {
            ...
            {
                if let (Some(param_values), Some(rho_values)) =
                    (param_arr.as_array(), rho_arr.as_array())
                {
                    let mut parameter = [KoalaBear::ZERO; PARAMETER_LEN];
                    for (idx, val) in param_values.iter().take(PARAMETER_LEN).enumerate() {
                    ...
                    for fe in &iteration_pos_output {
                        acc =
                            acc * BigUint::from(KoalaBear::ORDER_U64) + BigUint::from(fe.as_canonical_u32());
                    }
                    ...
                    let chunk_values = TopLevelPoseidonMessageHash::<15, 1, 15, 64, 8, 77, 2, 9, 5, 7>::apply(
                        &parameter,
                        epoch,
                        &rho,
                        &message_bytes,
                    ...
                    let hashes_vec: Vec<[KoalaBear; HASH_LEN_FE]> = sig_dbg_val
                        ...
                        .map(|domains| {
                            domains
                                .iter()
                                .map(|domain_val| {
                                    let domain_items = domain_val.as_array().expect("hash domain");
                                    let mut arr = [KoalaBear::ZERO; HASH_LEN_FE];
                                    for (j, value) in domain_items.iter().take(HASH_LEN_FE).enumerate() {
                    ...
                    for (idx, end) in hashes_vec.iter().take(3).enumerate() {
                        eprintln!(
                    ...
                    for (idx, value) in expected_root_values.iter().take(HASH_LEN_FE).enumerate() {
...
fn hex_array_to_numbers(value: &mut Value) {
    match value {
        Value::Array(arr) => {
            for val in arr.iter_mut() {
                hex_array_to_numbers(val);
            }
        }
        Value::Object(obj) => {

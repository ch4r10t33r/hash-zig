...
fn main() {
    ...
    let fixed = std::env::var("FIXED_DEBUG").ok().map(|v| v == "1").unwrap_or(false);
    let (parameter, prf_key) = if fixed {
        ...
        let param_json = serde_json::json!([
            param_u32[0] as u64, param_u32[1] as u64, param_u32[2] as u64, param_u32[3] as u64, param_u32[4] as u64
        ]);
        let parameter: <PoseidonTweakHash<5, 8, 2, 9, 64> as TweakableHash>::Parameter = serde_json::from_value(param_json).expect("failed to build TH::Parameter");
    ...
    let mut chain_ends: Vec<<PoseidonTweakHash<5, 8, 2, 9, 64> as TweakableHash>::Domain> = Vec::with_capacity(num_chains as usize);
    for chain_index in 0..num_chains {
        ...
        let mut current: <PoseidonTweakHash<5, 8, 2, 9, 64> as TweakableHash>::Domain = start_domain.into();
    ...
    let to_hex = |x: &<PoseidonTweakHash<5, 8, 2, 9, 64> as TweakableHash>::Domain| -> Vec<String> {
        x.iter().map(|f| format!("0x{:08x}", f.as_canonical_u64() as u32)).collect()
    };

    let leaf_hex = to_hex(&leaf);
    println!("RUST_LEAF_DOMAIN_EPOCH0:{}", serde_json::to_string(&leaf_hex).unwrap());

    // Also print parameter for comparison
    let param_hex: Vec<String> = parameter.iter().map(|f| format!("0x{:08x}", f.as_canonical_u64() as u32)).collect();
    println!("RUST_PARAMETER:{}", serde_json::to_string(&param_hex).unwrap());
}

...
pub fn main() !void {
    ...
    log.print("Testing Zig signing and verification with same keypair...\n", .{});
    ...
    log.print("Verifying signature with same keypair...\n", .{});
    const is_valid = try scheme.verify(&keypair.public_key, epoch, message_bytes, signature);

    log.print("Result: {}\n", .{is_valid});

    if (is_valid) {
        log.print("✅ SUCCESS: Zig signing and verification with same keypair works!\n", .{});
    } else {
        log.print("❌ FAILED: Zig verification failed with same keypair!\n", .{});
    }
}

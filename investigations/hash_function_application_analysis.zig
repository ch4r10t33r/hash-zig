...
pub fn main() !void {
    ...
    std.debug.print("=== Hash Function Application Analysis ===\n", .{});
    ...
    // Analyze hash function application for tree building
    std.debug.print("\n=== Hash Function Application Analysis ===\n", .{});

    // Simulate a tree building operation to analyze hash function application
    ...
    // Analyze the actual hash function application
    std.debug.print("\n=== Hash Function Application Analysis ===\n", .{});

    // Create the combined input array
    ...
    // Copy message (left + right child)
    for (0..8) |i| {
        combined_input[5 + 2 + i] = hash_zig.core.KoalaBearField{ .value = left_child[i] };
        combined_input[5 + 2 + 8 + i] = hash_zig.core.KoalaBearField{ .value = right_child[i] };
    }

    std.debug.print("Combined Input: {any}\n", .{combined_input});

    // Analyze the hash function application
    std.debug.print("\n=== Poseidon2 Hash Function Application ===\n", .{});
    ...
    std.debug.print("Hash Result: {any}\n", .{hash_result});

    // Analyze the specific differences in hash function application
    std.debug.print("\n=== Hash Function Application Differences ===\n", .{});
    std.debug.print("Rust: Uses separate iterators for left and right child\n", .{});
    std.debug.print("Zig: Concatenates left and right child into single array\n", .{});

    // Show the difference in input structure
    std.debug.print("Rust Input Structure:\n", .{});
    std.debug.print("  [parameter(5), tweak(2), left(8), right(8)]\n", .{});
    std.debug.print("Zig Input Structure:\n", .{});
    std.debug.print("  [parameter(5), tweak(2), left(8), right(8)]\n", .{});

    std.debug.print("Both use the same input structure!\n", .{});

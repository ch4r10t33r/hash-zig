...
pub fn main() !void {
    std.debug.print("=== Detailed Internal Layer Debugging ===\n", .{});
    ...
    // Now let's debug the V-based operations step by step
    std.debug.print("\n=== V-based Operations ===\n", .{});
    ...
    std.debug.print("\nFinal state after V-based operations (first 4 elements):\n", .{});
    for (0..4) |i| {
        std.debug.print("  state[{}] = {} (normal: {})\n", .{ i, state[i].value, state[i].toU32() });
    }
    ...
    poseidon2.apply_internal_layer_16(&state, rc);

...
pub const ChaCha12Rng = struct {
    ...
    pub const nonce = [_]u8{0} ** Cipher.nonce_length;

    pub const secret_seed_length = Cipher.key_length;

    /// Initialize from a 32-byte seed (matching Rust's StdRng::from_seed)
    pub fn init(secret_seed: [secret_seed_length]u8) ChaCha12Rng {
        var self = ChaCha12Rng{ .state = undefined, .offset = Cipher.key_length };
    ...
    pub fn random(self: *ChaCha12Rng) std.Random {
        return std.Random.init(self, fill);
    }

    // Refills the buffer with random bytes, overwriting the previous key.
    pub fn refill(self: *ChaCha12Rng) void {
    ...
    pub fn fill(self: *ChaCha12Rng, buf_: []u8) void {

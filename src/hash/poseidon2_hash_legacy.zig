...
pub const Poseidon2 = struct {
    ...
    /// Zero-allocation fast path for single-element compression (output_len_fe = 1)
    /// Used for Winternitz chain in-place hashing to avoid allocator issues in arenas
    pub fn compress1NoAlloc(
        self: *Poseidon2,
        input: []const FieldElement,
    ) FieldElement {
        _ = self;

        // Preconditions (debug-only): require at least 1 element and width bounds
        // Convert input to state (pad with zeros)
        var state: [width]u32 = std.mem.zeroes([width]u32);
        const copy_len: usize = @min(input.len, width);
        var i: usize = 0;
        while (i < copy_len) : (i += 1) {
            state[i] = input[i].toU32();
        }

        // Save original first element for feed-forward
        const original0: u32 = if (input.len > 0) input[0].toU32() else 0;

        // Convert to Montgomery form and permute
        var state_mont: [width]field_mod.MontFieldElem = undefined;
        i = 0;
        while (i < width) : (i += 1) {
            field_mod.toMontgomery(&state_mont[i], state[i]);
        ...
        // Extract first element, feed-forward, and reduce mod p
        const permuted0: u32 = field_mod.toNormal(state_mont[0]);
        const sum0: u64 = @as(u64, permuted0) + @as(u64, original0);
        const reduced0: u64 = sum0 % FieldElement.PRIME;
        return FieldElement.fromU64(reduced0);
    }
    ...
    pub fn sponge24(
        ...
        var stack_buffer: [200]FieldElement = undefined; // Stack buffer for small inputs (increased for leaf hashing)
